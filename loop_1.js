...
// choirs?
$: note(`
[-@2 f4@2 g#4@2 a#4@2 f4@2 -@2]
`).sound(choirSounds).n("<32>").phaser(4).vowel("<i>")
...
// Drums
$: s("<hh>").bank(hhbank).gain(.03).room(.2).cpm(rate*2)._pianoroll({labels:1})
$: s("<oh>").bank(dbank).gain(.03).room(.2).cpm(rate/4)._pianoroll({labels:1})
$: s("<- sd, - sd>").bank(dbank).gain(.08).room(.3).pan(".6").cpm(rate)._pianoroll({labels:1})
$: s("<- bd>").bank(dbank).gain(.25).room(.35).cpm(rate)._pianoroll({labels:1})
